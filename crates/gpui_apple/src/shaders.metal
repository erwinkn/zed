...
// Premultiply before the framebuffer clamps RGB. This preserves emissive
...
fragment float4 polychrome_sprite_fragment(
  ...
  constexpr sampler nearest_neighbor_sampler(mag_filter::nearest,
                                             min_filter::nearest);
  float4 sample = sprite.nearest_neighbor
                      ? atlas_texture.sample(nearest_neighbor_sampler,
                                             input.tile_position)
...
fragment float4 surface_fragment(SurfaceFragmentInput input [[stage_in]],
  ...
  float coverage = saturate(0.5 - quad_sdf(input.position.xy, surface.bounds, surface.corner_radii));
  float4 color = ycbcrToRGBTransform * ycbcr;
  color.a *= surface.opacity * coverage;
  return color;
}
...
fragment float4 rgba_surface_fragment(
  ...
  SurfaceBounds surface = surfaces[input.surface_id];
  float coverage = saturate(0.5 - quad_sdf(input.position.xy, surface.bounds, surface.corner_radii));
  // Preserve floating-point RGB above alpha until the final target blend.
  return texture.sample(texture_sampler, input.texture_position) * surface.opacity * coverage;
}
